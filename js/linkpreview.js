...
(function(jQuery) {
    ...
    LinkPreview.prototype = {
        ...
        init: function(element, options) {
            ...
            if (options && options.refreshButton) {
                this.$refreshButton = jQuery(options.refreshButton);
        ...
        initPreviewContainer: function() {
            if (this.getOption("previewContainer")) {
                this.$previewContainer = jQuery(this.options.previewContainer);
            } else {
        ...
        renderPreview: function(url, data, that) {
            ...
            var title = that.findTitleInDom($dom),
                ...
                noimage = 'http://uptownwhere.com/wp-content/themes/uptownwhere/images/no-image.jpg';

            // build dom elements
            var $title = jQuery("<a></a>").attr("href", url).text(title),
                $description = jQuery("<p></p>").text(description);

            var $spanRight;
            if (image) {
                var $image = jQuery("<img></img>").attr("src", image),
                    $spanLeft = jQuery("<div></div>").addClass("preview-image column fourth");
                $spanRight = jQuery("<div></div>").addClass("preview-description column three-fourths column-padding");
                $spanLeft
                    .append($image);
                that.$previewContainer
                    .append($spanLeft);
            } else {
                var $image = jQuery("<img></img>").attr("src", noimage),
                    $spanLeft = jQuery("<div></div>").addClass("preview-image column fourth");
                $spanRight = jQuery("<div></div>").addClass("preview-description column three-fourths column-padding");
                $spanLeft
                    .append($image);
 ...
 /*           if (imageSrc && !this.validateUrl(imageSrc)) {
        ...
        findFirstImageInBody: function($body) {
            ...
            $images.each(function() {
                ...
                if ($img.attr("height") && $img.attr("height") > 400 &&
                    $img.attr("width") && $img.attr("width") > 400) {
                    result = this.src;
                    return false;
                }
            });
    ...
    jQuery.fn.linkpreview = function (option) {
        return this.each(function () {
            ...
            if (!data) {
                $this.data('linkpreview', (data = new LinkPreview(this, jQuery.extend({}, jQuery.fn.linkpreview.defaults, options))));
            }
    ...
    jQuery.fn.linkpreview.defaults = {};
    
    jQuery.fn.linkpreview.Constructor = LinkPreview;
